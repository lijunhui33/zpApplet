			...
			<form @submit="formSubmit">
				<view class="edit_input">
					...
					<view class="box_input">
						<input type="text" v-model="info.s1" name="projectname" placeholder="请输入" placeholder-class="iright" style="text-align: right;"/>
						<view class="has"></view>
					</view>
				</view>
				...
				<view class="edit_input">
					<view class="label">
						<view class="red">*</view>
						<view class="">担任角色</view>
					</view>
					<view class="box_input">
						<input type="text" v-model="info.s2" name="role" placeholder="请输入" placeholder-class="iright" style="text-align: right;"/>
						<view class="has"></view>
					</view>
				</view>
				<view class="qs-line"></view>
				...
				<view class="edit_input" style="height: auto;align-items: flex-start;margin-top: 40rpx;margin-bottom: 40rpx;">
					<view class="label">
						<view class="red">*</view>
						<view class="">负责内容</view>
					</view>
					<view class="box_input">
						<textarea v-model="info.s3" placeholder="请输入负责内容" name="description" style="width: 400rpx;" placeholder-style="text-align:right"/>
						<view class="has"></view>
					</view>
				</view>
				<view class="qs-line"></view>
				<button form-type="submit" class="mysubmit">保存修改</button>
				<view class="deleteEdu" @click="delProject" v-if="!subType">删除该经历</view>
			</form>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				config:{
					...
					menuText:"编辑项目经历", //当type为3或4的时候icon右边的文字
				},
		...
		methods: {
			...
			async resumeProject(){
				...
				let res = await http.resumeProject(this.list_param,this.headers);
				if(res.state == 1){
			...
			async resumeDisplayProject(){
				let res = await http.resumeDisplayProject({id:this.id},this.headers);
				this.info = {s1:res[0].projectname,s2:res[0].role,s3:res[0].description};
				if(res[0].todate === "1"){
					this.date2 = "至今"
				}else{
			...
			async delProject(){
				this.headers['content-type'] = 'application/x-www-form-urlencoded';
				let res = await http.delProject({id:this.id},this.headers);
				if(res.status == 200){
