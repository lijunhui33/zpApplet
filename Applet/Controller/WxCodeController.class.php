...
class WxCodeController extends BaseController{
     ...
     * @return string
     */
    private function signature($WxTokenheader, $payload, $secret = 'code_sms')
    {
     ...
     * @return array|string
     */
    public function checkToken($jwt, $key = 'code_sms'){
     ...
     * @return array|string
     */
    public function regcheck($jwt,$vcode,$key = 'code_sms'){
        $access_token = explode('.', $jwt);  
        if (count($access_token) != 3)        
        	return 'token invalid';        
        $WxTokenheader64 = $access_token[0];
     ...
     * @return null
     */
    public function get_vcode_Token()
    {
        $token = null;        
        if (isset($_SERVER['HTTP_VCODE']))            
        $token = $_SERVER['HTTP_VCODE'];    
        return $token;
    }
